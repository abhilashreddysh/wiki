...
The file type is represented by the first character of the permission string, and it can be one of the following:
...
So, for example, a file permission string starting with "d" would indicate a directory, and the permission number would be calculated in the same way as for a regular file.
